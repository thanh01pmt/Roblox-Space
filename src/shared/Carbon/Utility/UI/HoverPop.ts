...
export function HoverPopUp(button: GuiButton, pop: number, spd: number, defaultGoal: AnimatedButtonDefault) {
...
export function HoverPopDown(button: GuiButton, spd: number, defaultGoal: AnimatedButtonDefault) {
    const info = new TweenInfo(spd, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut);
    Tween(button, info, defaultGoal);
}